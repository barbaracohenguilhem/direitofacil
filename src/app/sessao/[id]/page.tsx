...
export default function SessionPage() {
  ...
  if (!question || !learner) {
    return <main className="min-h-dvh bg-[#171614] text-white flex items-center justify-center"><div className="text-sm text-white/50">Preparando sua sessão…</div></main>;
  }
    ...
    <main className="min-h-dvh bg-[#171614] text-[#f8f6f2]">
      <header className="border-b border-white/10 px-5 py-4 md:px-8">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          ...
          <div className="text-xs text-white/45">Sessão de hoje · {index + 1} de {plan.length}</div>
        </div>
      </header>
      ...
      <div className="mx-auto grid min-h-[calc(100dvh-61px)] max-w-6xl gap-8 px-5 py-8 lg:grid-cols-[1fr_320px] lg:py-12">
        <section className="flex min-h-[620px] flex-col rounded-[28px] bg-[#f8f6f2] p-6 text-[#1c1a18] md:p-10">
          <div className="mb-8 flex items-center justify-between gap-6">
            ...
            <div className="flex gap-1">{plan.map((_, i) => <span key={i} className={`h-1.5 w-8 rounded-full ${i <= index ? 'bg-[#1c1a18]' : 'bg-[#ded9d2]'}`} />)}</div>
          </div>
          ...
          {stage === 'materials' && (
            <div className="fade-in flex flex-1 flex-col">
              ...
              <p className="mt-5 max-w-2xl text-sm leading-6 text-[#77716a]">Seu próximo percurso já foi recalculado em silêncio. Antes de sair, leve só o que vale revisar.</p>
        ...
        <aside className="flex flex-col gap-4">
          ...
          <div className="rounded-[24px] border border-white/10 p-5">
            ...
            <p className="mt-2 text-xs leading-5 text-white/40">{activity.reason === 'reinforce' ? 'Este conceito reapareceu porque ainda merece reforço.' : activity.reason === 'retention' ? 'Uma revisão entrou no momento certo.' : activity.reason === 'transfer' ? 'A mesma lógica voltou em outro contexto.' : 'Um conceito novo entrou no percurso.'}</p>
          ...
          <p className="px-2 text-[11px] leading-5 text-white/25">Na versão final, a explicação do motivo acima não será exibida ao aluno. Ela aparece aqui apenas para validar o protótipo do motor adaptativo.</p>
        </aside>
      </div>
    </main>
  );
